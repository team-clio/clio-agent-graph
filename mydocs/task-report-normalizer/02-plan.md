...
#### (a) 오류 관련 정보를 하나의 문자열 목록으로 저장

- 초기 구현은 단순하다.
- RM의 exact/fingerprint 검색과 IA의 stack frame 탐색에서 다시 파싱해야 한다.

#### (b) 예외 타입·메시지·코드·stack frame을 분리

- 후속 검색기가 각 신호를 목적에 맞게 사용한다.
- NM 출력 스키마가 조금 커진다.

**추천: (b).** 최소한 `exception_type`, `message`, `error_code`, `stack_frames`를 분리한다. 각 값은 독립적인
근거를 가진다.
...
**추천: (b).** NM은 추출기다. 원인 추론은 IA의 책임이며, 찾지 못한 값은 `missing_fields`로 보낸다. 초기에는
LLM 자기평가 confidence를 핵심 계약에 넣지 않는다.
